...
if VERSION < v"0.5.0-dev+4724"
    ...
    using Base.Broadcast: newindexer, _broadcast!, newindex
    ...
    `base/broadcast.jl`.
    """ ->
    @inline function Base.broadcast!(f, Z::NullableArray, Xs::NullableArray...; lift=false)
    # @inline function Base.broadcast!{nargs}(f, Z::NullableArray, Xs::Vararg{NullableArray, nargs})
        nargs = length(Xs)
        sz = size(Z)
